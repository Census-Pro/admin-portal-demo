...
export function EditOperatorModal({
  ...
  const [cidNo, setCidNo] = useState(operator.cidNo);
  const [error, setError] = useState('');

  useEffect(() => {
    if (open) {
      setCidNo(operator.cidNo);
      setError('');
    }
  }, [open, operator]);
  ...
  const handleCidBlur = async () => {
    ...
    if (cidNo && cidNo.length === 11 && cidNo !== operator.cidNo) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast.success(result.message || 'Operator updated successfully');
      onSuccess?.();
    } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the operator's CID number.
          </DialogDescription>
        </DialogHeader>
