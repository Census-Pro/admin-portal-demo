...
export function AddGenderModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let result;
      if (initialData) {
        result = await updateGender(initialData.id, { name });
      } else {
        result = await createGenders({ name });
      }
      ...
      if (result && !result.error) {
        ...
        toast.error(
          result?.message ||
            `Failed to ${initialData ? 'update' : 'create'} gender`
        );
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{initialData ? 'Edit' : 'Add New'} Gender</DialogTitle>
          <DialogDescription>
            {initialData
              ? 'Update the gender details.'
              : 'Create a new gender in the system.'}
          </DialogDescription>
        </DialogHeader>
