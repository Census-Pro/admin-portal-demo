...
import { useAnnouncementCategoriesTableFilters } from './use-announcement-categories-table-filters';
...
export function CategoriesTable({ data, addButton }: CategoriesTableProps) {
  ...
  const {
    ...
    setPage,
    setSearchQuery
  } = useAnnouncementCategoriesTableFilters();
  ...
  const handleReorder = async (newOrder: AnnouncementCategory[]) => {
    const oldItems = [...items];
    setItems(newOrder);

    try {
      const updates = newOrder.map((item, index) =>
        updateAnnouncementCategory(item.id, { order: index })
      );
      ...
      if (hasError) {
        console.error(
          '[handleReorder] Some announcement category order updates failed'
        );
        toast.error('Failed to update some announcement category orders');
        setItems(oldItems);
      } else {
        toast.success('Announcement category order updated successfully');
        // Use router.refresh() instead of full page reload
        router.refresh();
      }
    } catch (error) {
      console.error('[handleReorder] Exception:', error);
      toast.error('Error updating announcement category order');
      setItems(oldItems);
    }
  };
